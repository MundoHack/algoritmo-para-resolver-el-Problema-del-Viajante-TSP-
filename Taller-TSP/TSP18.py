...
def run_tsp_and_display(graph, start_node, display_area):
    ...
    for heuristic_name, heuristic_func in heuristics.items():
        ...
        result_text = f"{heuristic_name} Heuristic - Cost: {cost}, Time: {elapsed_time:.4f}s"
...
def execute_tsp_from_input(edges_window, root, entry_widgets, node_names):
    ...
    display_area = Text(root, width=60, height=20)
    display_area.grid(row=1, column=0, padx=10, pady=10)
...
def main():
    root = Tk()
    root.title("Algoritmo de Vendedor Viajero (TSP)")
    
    Button(root, text="Ingresar Conexiones", command=lambda: show_edge_input_interface(root)).grid(row=0, column=0, padx=10, pady=10)
