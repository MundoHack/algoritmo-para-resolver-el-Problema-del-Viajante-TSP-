...
def show_edge_input_interface(root):
    ...
    def create_connection_entries(node_entries):
        ...
        # Crear conexiones necesarias entre todos los pares únicos de nodos
        ...
        entry_widgets = []
        for i, (node1, node2) in enumerate(combinations(node_names, 2)):  # Todas las combinaciones únicas de pares de nodos
            frame = Frame(connection_frame)
            frame.pack()
            entry1 = Entry(frame, width=10)
            entry2 = Entry(frame, width=10)
            weight_entry = Entry(frame, width=5)
            entry1.insert(0, node1)  # Rellenar con el nombre del nodo
            entry2.insert(0, node2)  # Rellenar con el nombre del nodo
...
def display_results(root, graph):
    ...
    nx.draw(graph, pos, ax=ax, with_labels=True, node_color='lightblue', font_size=10)
