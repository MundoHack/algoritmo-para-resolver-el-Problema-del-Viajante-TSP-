...
def plot_graph(graph, path):
    ...
    plt.figure(figsize=(8, 5))  # Cambiar el tamaño del gráfico
    ...
    if path:
        path_edges = list(zip(path, path[1:] + [path[0]]))  # Añadir el camino de regreso al inicio
        nx.draw_networkx_edges(graph, pos, edgelist=path_edges, edge_color='r', width=3)
    
    plt.title("Grafo y Camino TSP")
    plt.show()
...
def execute_tsp_from_input(edges_window, root, entry_widgets, node_names):
    ...
    start_node = node_names[0]  # Puedes elegir otro nodo si deseas
