...
# Heurística MST adaptada al cálculo de A*
def mst_heuristic(graph, current_node, unvisited_nodes):
    # Construir MST para el conjunto de nodos no visitados
    ...
    # Calcular el costo mínimo desde el nodo actual a cualquiera de los nodos en el MST
    min_edge_cost = min(
        graph[current_node][n]["weight"] for n in unvisited_nodes if graph.has_edge(current_node, n)
    )
    
    # Suma del costo del MST más el costo mínimo al MST
