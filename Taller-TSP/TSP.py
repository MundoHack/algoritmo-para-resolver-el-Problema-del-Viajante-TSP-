...
def astar_tsp(graph, start_node, heuristic_func, pheromone_map=None):
    ...
    while frontier:
        ...
        heuristic_cost = heuristic_func(graph, current_node, unvisited_nodes, pheromone_map)
...
def run_tsp_examples():
    ...
    aco_path, aco_cost = astar_tsp(graph, start_node, lambda g, cn, un, pm: ant_colony_heuristic(g, cn, un, pm), pheromone_map)
    print(f"ACO Heuristic Path: {aco_path}, Cost: {aco_cost}")
    
    # 3. A* con Heurística Combinada MST + ACO
    combined_path, combined_cost = astar_tsp(graph, start_node, lambda g, cn, un, pm: combined_heuristic(g, cn, un, pm, alpha=0.5), pheromone_map)
